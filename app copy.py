...
st.markdown("---")
df = pd.DataFrame()
options = ["Mongo DB", "Excel Sheets"]
selected_option_tab = st.selectbox('Select an option for Input', options, index=None,key="tab")

if selected_option_tab:
    if selected_option_tab == "Excel Sheets":
        flag = 0
    ...
    if selected_option_tab == "Mongo DB":
        flag = 0
